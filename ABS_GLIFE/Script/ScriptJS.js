...
function JSAddItem_ASP() {
    ...
    if (Pcnt > 0) {
        myprm = confirm("WARNING: Records will be Added!" +
          "\nAre you sure you want to Add these selected item(s)?");
        if (myprm == true) {
            document.forms["Form1"].elements["txtAction"].value = "Add_Item";
            document.forms["Form1"].submit;
        }
    }
    else {
        Pcnt = 0;
        alert("You must select an item to Add. Nothing is selected to Add...");
    }
