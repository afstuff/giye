...
Partial Class Policy_PRG_LI_GRP_ADD_MEMBERS
    Inherits System.Web.UI.Page

    'Adding Reference

    'In order to convert the Word Documents to HTML you will need Microsoft Word Object Library. 
    ...
    '    savePath = HttpRuntime.AppDomainAppPath & myFld & "\"
    ...
    '    Me.txtMsg.Text = "Upload Error. Reason: " & ex.Message
    '    Exit Sub
    'End Try
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        strTableName = "TBIL_GRP_ADD_COVER_MEMBERS"
        ...
        Try
            'Me.txtBatch_Num.Text = CType(Request.QueryString("optbatno"), String)
            Me.txtBatch_Num.Text = CType(Session("optbatno"), String)
        Catch ex As Exception
        ...
        If Not (Page.IsPostBack) Then
            ...
            Me.txtXLS_Data_End_No.Text = "100000"
            ...
            If Trim(strF_ID) <> "" Then
                ...
                If oAL.Item(0) = "TRUE" Then
                    ...
                    'Me.txtAdd_No_Prem_Yr.Text = oAL.Item(19)
                    ...
                    ' OPTIONS FOR PREMIUM RATE TYPE, NOT FOR ADD COVER MEMBER HEADER
            ...
            strSQL = ""
            strSQL = strSQL & "SELECT ADD_COV.TBIL_POL_ADD_COVER_CD AS MyFld_Value "
            strSQL = strSQL & ", COV.TBIL_COV_DESC AS MyFld_Text"
            strSQL = strSQL & " FROM " & strTable & " AS ADD_COV"
            strSQL = strSQL & " LEFT JOIN TBIL_COVER_DET AS COV ON COV.TBIL_COV_CD = ADD_COV.TBIL_POL_ADD_COVER_CD"
            strSQL = strSQL & " WHERE ADD_COV.TBIL_POL_ADD_FILE_NO = '" & RTrim(Me.txtFileNum.Text) & "'"
            strSQL = strSQL & " AND ADD_COV.TBIL_POL_ADD_PROP_NO = '" & RTrim(Me.txtQuote_Num.Text) & "'"
            'strSQL = strSQL & " AND ADD_COV.TBIL_POL_ADD_POLY_NO = '" & RTrim(Me.txtPolNum.Text) & "'"
            'strSQL = strSQL & " AND ADD_COV.TBIL_POL_ADD_PRDCT_CD = '" & RTrim(Me.txtProduct_Num.Text) & "'"
            strSQL = strSQL & " AND ADD_COV.TBIL_POL_ADD_MDLE IN('GRP','G')"
            strSQL = strSQL & " ORDER BY ADD_COV.TBIL_POL_ADD_PRDCT_CD, COV.TBIL_COV_DESC"

            'Call gnProc_Populate_Box("GL_COVER_LIST_OTHERS", RTrim(Me.txtProduct_Num.Text), Me.cboCover_Name)
            Call gnProc_Populate_Box("GL_COVER_LIST_ADD_COVER_HEADER", RTrim(Me.txtProduct_Num.Text), Me.cboCover_Name, "", strSQL)
            Call gnProc_Populate_Box("GL_PLAN_LIST", RTrim(Me.txtProduct_Num.Text), Me.cboPlan_Name)
    ...
    Protected Sub cboCover_Name_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboCover_Name.SelectedIndexChanged
        Call gnGET_SelectedItem(Me.cboCover_Name, Me.txtCover_Num, Me.txtCover_Name, Me.lblMsg)
        If Trim(Me.txtCover_Num.Text) <> "" Then
            Call Proc_DoLoad_Add_Cover(RTrim("LOAD_ADD_COVER_HDR"))
        End If
    ...
    Protected Sub cmdFile_Upload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdFile_Upload.Click
        ...
        'Invalid column name 'TBIL_POL_MEMB_STATUS'. 
        'Invalid column name 'TBIL_POL_MEMB_BATCH_NO'. 
        'Invalid column name 'TBIL_POL_MEMB_PROP_NO'. 
        'Invalid column name 'TBIL_POL_MEMB_PROP_NO'
        ...
        Select Case UCase(Trim(Me.txtData_Source_SW.Text))
            ...
            Case "U"
                ...
                Try
                    ...
                    'strFilePath = Server.MapPath(strPATH & Me.txtFile_Upload.Text)
                ...
                If Me.chkData_Source.Checked = True Then
                    ...
                    Call Proc_DoLoad_Add_Cover(RTrim("OLE"))
                Else
                    'Call Proc_DoSave_Upload()
                    Call Proc_DoLoad_Add_Cover(RTrim("TBL"))
                End If
                'Me.tr_file_upload.Visible = False
    ...
    Private Sub Proc_DoLoad_Add_Cover(ByVal pvCODE As String)
        ...
        strSQL = strSQL & " AND TBIL_POL_ADD_COVER_CD = '" & RTrim(Me.txtCover_Num.Text) & "'"
        ...
        Try
            ...
            Do While objMem_DR.Read()
                ...
                'pvListItem.Value = RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_SA_GRP_MEMB") & vbNullString, String))
                ' use 
                'pvListItem.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_SA_GRP_MEMB") & vbNullString, String))

                'TBIL_POL_ADD_PREM_SA_GRP_MEMB()
                'TBIL_POL_ADD_SA_RT_AMT_CD()
                'TBIL_POL_ADD_PCENT_SA()
                'TBIL_POL_ADD_SA_LC()

                'TBIL_POL_ADD_COVER_CD

                'TBIL_POL_ADD_SA_RT_AMT_CD
                'TBIL_POL_ADD_PREM_RT_AMT_CD

                strCover_Num = RTrim(CType(objMem_DR("TBIL_POL_ADD_COVER_CD") & vbNullString, String))

                Me.txtPrem_Rate_Code.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_RT_CD") & vbNullString, String))
                Call Proc_DDL_Get(Me.cboPrem_Rate_Code, Me.txtPrem_Rate_Code.Text)

                Me.txtPrem_Rate.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_RATE") & vbNullString, String))
                Me.txtPrem_Rate_Per.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_RATE_PER") & vbNullString, String))


                Me.txtPrem_Rate_TypeNum.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_RT_AMT_CD") & vbNullString, String))
                Call Proc_DDL_Get(Me.cboPrem_Rate_Type, Me.txtPrem_Rate_TypeNum.Text)

                Me.txtAdd_Prem_Fixed_Rate.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_FX_RT") & vbNullString, String))
                Me.txtAdd_Prem_Fixed_Rate_PerNum.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_FX_RT_PER") & vbNullString, String))

                'Me.txtAdd_Prem_Amt.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_AMT") & vbNullString, String))
                'Me.txtSum_Assured.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_SA_LC") & vbNullString, String))

                Me.txtAdd_Prem_Rate_Applied_On_Num.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_RATE_APPLY") & vbNullString, String))
                Call Proc_DDL_Get(Me.cboAdd_Prem_Rate_Applied_On, Me.txtAdd_Prem_Rate_Applied_On_Num.Text)

                Me.txtAdd_SA_RT_AMT_CD.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_SA_RT_AMT_CD") & vbNullString, String))
                Me.txtAdd_PCENT_SA.Text = RTrim(CType(objMem_DR("TBIL_POL_ADD_PCENT_SA") & vbNullString, String))


                Select Case RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_SA_GRP_MEMB") & vbNullString, String))
                    Case "I"
                        Select Case RTrim(CType(objMem_DR("TBIL_POL_ADD_PREM_RT_AMT_CD") & vbNullString, String))
                            Case "A"
                            Case "F"
                            Case "R"
                        End Select
        ...
        Try
            ...
            Do While objMem_DR.Read()
                ...
                If UCase(Trim(pvFT)) = "Y" Then
                    pvFT = "N"
                    If Trim(Me.txtBatch_Num.Text) = "" Then
                        pvBatNum = RTrim(CType(objMem_DR("TBIL_ADD_COV_MEMB_BATCH_NO") & vbNullString, String))
                    End If
                End If
                ...
                pvListItem.Value = RTrim(CType(objMem_DR("TBIL_ADD_COV_MEMB_BATCH_NO") & vbNullString, String))
                pvListItem.Text = RTrim(CType(objMem_DR("TBIL_ADD_COV_MEMB_BATCH_NO") & vbNullString, String))
    ...
    Private Function Proc_Batch_Check() As Boolean
        ...
        Try
            ...
            mystr_sql = ""
            mystr_sql = "SELECT TOP 1 TBIL_ADD_COV_MEMB_PROP_NO from " & strTable
            mystr_sql = mystr_sql & " where TBIL_ADD_COV_MEMB_PROP_NO = '" & RTrim(Me.txtQuote_Num.Text) & "'"
            'mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_POLY_NO = '" & RTrim(Me.txtPolNum.Text) & "'"
            'mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_FILE_NO = '" & RTrim(Me.txtFileNum.Text) & "'"
            mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_BATCH_NO = '" & RTrim(Me.txtBatch_Num.Text) & "'"
            mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_CD = '" & RTrim(Me.txtCover_Num.Text) & "'"
            mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_STATUS IN('P')"
    ...
    Private Sub Proc_DataBind()
        ...
        strSQL = strSQL & " FROM " & strTable & " "
        strSQL = strSQL & " WHERE TBIL_ADD_COV_MEMB_FILE_NO = '" & RTrim(strF_ID) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_PROP_NO = '" & RTrim(strQ_ID) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_BATCH_NO = '" & RTrim(Me.txtBatch_Num.Text) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_CD = '" & RTrim(Me.txtCover_Num.Text) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_MDLE IN('G')"
        strSQL = strSQL & " ORDER BY TBIL_ADD_COV_MEMB_FILE_NO, TBIL_ADD_COV_MEMB_BATCH_NO, TBIL_ADD_COV_MEMB_SNO"
    ...
    Protected Sub DoProc_Premium_RateType_Change()
        ...
        Select Case UCase(Me.txtPrem_Rate_TypeNum.Text)
            Case "A"
    ...
    Private Sub Proc_DoDelete()
        ...
        strSQL = strSQL & " WHERE TBIL_ADD_COV_MEMB_FILE_NO = '" & RTrim(strREC_ID) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_PROP_NO = '" & RTrim(Me.txtQuote_Num.Text) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_REC_ID = '" & Val(Trim(Me.txtRecNo.Text)) & "'"
    ...
    Protected Sub Proc_DoDelete_Record()
        ...
        strSQL = "DELETE FROM " & strTable
        strSQL = strSQL & " WHERE TBIL_ADD_COV_MEMB_FILE_NO = '" & RTrim(strREC_ID) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_PROP_NO = '" & RTrim(txtQuote_Num.Text) & "'"
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_REC_ID = " & Val(RTrim(Me.txtRecNo.Text)) & ""
    ...
    Private Sub Proc_DoNew()
        ...
        Call Proc_DDL_Get(Me.cboPrem_Rate_Type, RTrim("*"))
        ...
        Me.lblBatch_Num.Enabled = False
        Me.txtBatch_Num.Enabled = False
        Me.cboBatch_Num.Visible = False
        'Me.cmdGetBatch.Visible = False
    ...
    Private Sub Proc_DoSave()
        ...
        If Me.txtMember_SN.Text = "" Then
            Me.lblMsg.Text = "Missing " & Me.lblMember_SN.Text
            FirstMsg = "Javascript:alert('" & Me.lblMsg.Text & "')"
            Exit Sub
        End If
...
Proc_Skip_ANB:
        ...
        Select Case UCase(Trim(Me.txtPrem_Rate_TypeNum.Text))
            ...
            Case "N"    ' no premium
                Me.txtPrem_Rate.Text = "0"
            Case "T", "R"    ' premium is from table
        ...
        If MemJoin_Date > GenStart_Date And dblPrem_Amt <> 0 And intDays_Diff <> 0 Then
            dblPrem_Amt_ProRata = (dblPrem_Amt / intRisk_Days) * intDays_Diff
        End If
        ...
        strSQL = strSQL & " WHERE TBIL_ADD_COV_MEMB_FILE_NO = '" & RTrim(txtFileNum.Text) & "'"
        'strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_PROP_NO = '" & RTrim(txtQuote_Num.Text) & "'"
        'If Val(LTrim(RTrim(Me.txtRecNo.Text))) <> 0 Then
        strSQL = strSQL & " AND TBIL_ADD_COV_MEMB_REC_ID = '" & Val(RTrim(txtRecNo.Text)) & "'"
        'End If
        ...
        Try
            ...
            If obj_DT.Rows.Count = 0 Then
                ...
                drNewRow("TBIL_ADD_COV_MEMB_FILE_UPLOAD_SW") = RTrim(Me.txtData_Source_SW.Text)
                drNewRow("TBIL_ADD_COV_MEMB_FILE_UPLOAD_NAME") = RTrim(Me.txtFile_Upload.Text)
                drNewRow("TBIL_ADD_COV_MEMB_MDLE") = RTrim("G")

                drNewRow("TBIL_ADD_COV_MEMB_STATUS") = "Q"

                drNewRow("TBIL_ADD_COV_MEMB_FILE_NO") = RTrim(Me.txtFileNum.Text)
                drNewRow("TBIL_ADD_COV_MEMB_PROP_NO") = RTrim(Me.txtQuote_Num.Text)
                'drNewRow("TBIL_ADD_COV_MEMB_POLY_NO") = RTrim(Me.txtPolNum.Text)

                drNewRow("TBIL_ADD_COV_MEMB_CD") = RTrim(Me.txtCover_Num.Text)

                drNewRow("TBIL_ADD_COV_MEMB_BATCH_NO") = RTrim(Me.txtBatch_Num.Text)

                drNewRow("TBIL_ADD_COV_MEMB_SNO") = Val(Me.txtMember_SN.Text)
                drNewRow("TBIL_ADD_COV_MEMB_CAT") = RTrim(Me.txtGender.Text)

                If Trim(Me.txtMember_DOB.Text) <> "" Then
                    drNewRow("TBIL_ADD_COV_MEMB_BDATE") = dteDOB
                End If
                drNewRow("TBIL_ADD_COV_MEMB_AGE") = Val(Me.txtDOB_ANB.Text)

                If Trim(Me.txtStart_Date.Text) <> "" Then
                    drNewRow("TBIL_ADD_COV_MEMB_FROM_DT") = dteStart
                End If
                If Trim(Me.txtEnd_Date.Text) <> "" Then
                    drNewRow("TBIL_ADD_COV_MEMB_TO_DT") = dteEnd
                End If

                drNewRow("TBIL_ADD_COV_MEMB_TENOR") = Val(Me.txtPrem_Period_Yr.Text)
                drNewRow("TBIL_ADD_COV_MEMB_DESIG") = Left(RTrim(Me.txtDesignation_Name.Text), 40)
                drNewRow("TBIL_ADD_COV_MEMB_NAME") = Left(RTrim(Me.txtMember_Name.Text), 98)

                drNewRow("TBIL_ADD_COV_MEMB_SA_FACTOR") = Val(Trim(Me.txtPrem_SA_Factor.Text))
                drNewRow("TBIL_ADD_COV_MEMB_TOT_EMOLUMENT") = CDbl(Trim(Me.txtTotal_Emolument.Text))
                drNewRow("TBIL_ADD_COV_MEMB_SA") = CDbl(Trim(Me.txtSum_Assured.Text))
                drNewRow("TBIL_ADD_COV_MEMB_MEDICAL_YN") = RTrim(Me.txtMedical_YN.Text)

                drNewRow("TBIL_ADD_COV_MEMB_RATE") = RTrim(Trim(Me.txtPrem_Rate.Text))
                drNewRow("TBIL_ADD_COV_MEMB_RATE_PER") = Val(Trim(Me.txtPrem_Rate_Per.Text))

                drNewRow("TBIL_ADD_COV_MEMB_PREM") = CDbl(dblPrem_Amt)
                drNewRow("TBIL_ADD_COV_MEMB_PRO_RATE_PREM") = CDbl(dblPrem_Amt_ProRata)
                drNewRow("TBIL_ADD_COV_MEMB_LOAD") = CDbl(dblLoad_Amt)

                drNewRow("TBIL_ADD_COV_MEMB_FLAG") = "A"
                drNewRow("TBIL_ADD_COV_MEMB_OPERID") = CType(myUserIDX, String)
                drNewRow("TBIL_ADD_COV_MEMB_KEYDTE") = Now
            ...
            Else
                ...
                With obj_DT
                    ...
                    .Rows(0)("TBIL_ADD_COV_MEMB_FILE_UPLOAD_SW") = RTrim(Me.txtData_Source_SW.Text)
                    .Rows(0)("TBIL_ADD_COV_MEMB_FILE_UPLOAD_NAME") = RTrim(Me.txtFile_Upload.Text)
                    .Rows(0)("TBIL_ADD_COV_MEMB_MDLE") = RTrim("G")

                    .Rows(0)("TBIL_ADD_COV_MEMB_FILE_NO") = RTrim(Me.txtFileNum.Text)
                    .Rows(0)("TBIL_ADD_COV_MEMB_PROP_NO") = RTrim(Me.txtQuote_Num.Text)
                    '.Rows(0)("TBIL_ADD_COV_MEMB_POLY_NO") = RTrim(Me.txtPolNum.Text)

                    .Rows(0)("TBIL_ADD_COV_MEMB_CD") = RTrim(Me.txtCover_Num.Text)

                    .Rows(0)("TBIL_ADD_COV_MEMB_BATCH_NO") = RTrim(Me.txtBatch_Num.Text)

                    .Rows(0)("TBIL_ADD_COV_MEMB_SNO") = Val(Me.txtMember_SN.Text)
                    .Rows(0)("TBIL_ADD_COV_MEMB_CAT") = RTrim(Me.txtGender.Text)

                    If Trim(Me.txtMember_DOB.Text) <> "" Then
                        .Rows(0)("TBIL_ADD_COV_MEMB_BDATE") = dteDOB
                    End If
                    .Rows(0)("TBIL_ADD_COV_MEMB_AGE") = Val(Me.txtDOB_ANB.Text)

                    If Trim(Me.txtStart_Date.Text) <> "" Then
                        .Rows(0)("TBIL_ADD_COV_MEMB_FROM_DT") = dteStart
                    End If
                    If Trim(Me.txtEnd_Date.Text) <> "" Then
                        .Rows(0)("TBIL_ADD_COV_MEMB_TO_DT") = dteEnd
                    End If

                    .Rows(0)("TBIL_ADD_COV_MEMB_TENOR") = Val(Me.txtPrem_Period_Yr.Text)
                    .Rows(0)("TBIL_ADD_COV_MEMB_DESIG") = Left(RTrim(Me.txtDesignation_Name.Text), 40)
                    .Rows(0)("TBIL_ADD_COV_MEMB_NAME") = Left(RTrim(Me.txtMember_Name.Text), 98)

                    .Rows(0)("TBIL_ADD_COV_MEMB_SA_FACTOR") = Val(Trim(Me.txtPrem_SA_Factor.Text))
                    .Rows(0)("TBIL_ADD_COV_MEMB_TOT_EMOLUMENT") = CDbl(Trim(Me.txtTotal_Emolument.Text))
                    .Rows(0)("TBIL_ADD_COV_MEMB_SA") = CDbl(Trim(Me.txtSum_Assured.Text))
                    .Rows(0)("TBIL_ADD_COV_MEMB_MEDICAL_YN") = RTrim(Me.txtMedical_YN.Text)

                    .Rows(0)("TBIL_ADD_COV_MEMB_RATE") = CDbl(Trim(Me.txtPrem_Rate.Text))
                    .Rows(0)("TBIL_ADD_COV_MEMB_RATE_PER") = Val(Trim(Me.txtPrem_Rate_Per.Text))

                    .Rows(0)("TBIL_ADD_COV_MEMB_PREM") = CDbl(dblPrem_Amt)
                    .Rows(0)("TBIL_ADD_COV_MEMB_PRO_RATE_PREM") = CDbl(dblPrem_Amt_ProRata)
                    .Rows(0)("TBIL_ADD_COV_MEMB_LOAD") = CDbl(dblLoad_Amt)

                    .Rows(0)("TBIL_ADD_COV_MEMB_FLAG") = "C"
                    '.Rows(0)("TBIL_ADD_COV_MEMB_OPERID") = CType(myUserIDX, String)
                    '.Rows(0)("TBIL_ADD_COV_MEMB_KEYDTE") = Now
                End With
    ...
    Private Sub Proc_DoSave_OLE()
        ...
        my_Cover_Num = Me.txtCover_Num.Text
...
MyLoop_Start:
        ...
        If Trim(sFT) = "Y" Then
            ...
            mystr_sql = ""
            mystr_sql = "delete from TBIL_GRP_ADD_COVER_MEMBERS"
            mystr_sql = mystr_sql & " where TBIL_ADD_COV_MEMB_FILE_NO = '" & RTrim(my_File_Num) & "'"
            mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_PROP_NO = '" & RTrim(my_Prop_Num) & "'"
            mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_BATCH_NO = '" & RTrim(Me.txtBatch_Num.Text) & "'"
            mystr_sql = mystr_sql & " and TBIL_ADD_COV_MEMB_CD = '" & RTrim(Me.txtCover_Num.Text) & "'"
            myole_cmd = New OleDbCommand(mystr_sql, myole_con)
            myole_cmd.CommandType = CommandType.Text
        ...
        Select Case UCase(Trim(Me.txtPrem_Rate_TypeNum.Text))
            ...
            Case "T"
                ...
                Else
                    'Me.txtPrem_Rate.Text = myRetValue.ToString
                    dblPrem_Rate = CDbl(Trim(myRetValue.ToString))
                    dblPrem_Rate_Per = Val(Trim(Me.txtPrem_Rate_Per.Text))
                End If
        ...
        mystr_sql = "SPGL_TBIL_GRP_ADD_COVER_MEMBERS_INSERT"
        ...
        myole_cmd.Parameters.AddWithValue("@p05A", RTrim(my_Cover_Num))
        myole_cmd.Parameters.AddWithValue("@p05B", RTrim(my_Batch_Num))
        myole_cmd.Parameters.AddWithValue("@p05C", RTrim(my_Staff_Num))
...
MyLoop_End:
    ...
    Private Function Proc_DoOpenRecord(ByVal FVstrGetType As String, ByVal FVstrRefNum As String, Optional ByVal FVstrRecNo As String = "", Optional ByVal strSearchByWhat As String = "FILE_NUM") As String
        ...
        strSQL = strSQL & " WHERE BEN_TBL.TBIL_ADD_COV_MEMB_FILE_NO = '" & RTrim(strREC_ID) & "'"
        If Val(LTrim(RTrim(FVstrRecNo))) <> 0 Then
            strSQL = strSQL & " AND BEN_TBL.TBIL_ADD_COV_MEMB_REC_ID = '" & Val(FVstrRecNo) & "'"
        End If
        ...
        strSQL = "SPGL_GET_ADD_COVER_MEMBERS"
        ...
        If (objOLEDR.Read()) Then
            ...
            Me.txtFileNum.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_FILE_NO") & vbNullString, String))
            'Call Proc_DDL_Get(Me.ddlGroup, RTrim(Me.txtGroupNum.Text))
            Me.txtRecNo.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_REC_ID") & vbNullString, String))

            Me.txtQuote_Num.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_PROP_NO") & vbNullString, String))
            Me.txtPolNum.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_POLY_NO") & vbNullString, String))

            Me.txtCover_Num.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_CD") & vbNullString, String))
            Call gnProc_DDL_Get(Me.cboCover_Name, RTrim(Me.txtCover_Num.Text))

            Me.txtData_Source_SW.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_FILE_UPLOAD_SW") & vbNullString, String))
            ...
            Me.txtFile_Upload.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_FILE_UPLOAD_NAME") & vbNullString, String))

            Me.txtBatch_Num.Text = RTrim(objOLEDR("TBIL_ADD_COV_MEMB_BATCH_NO") & vbNullString)
            'Me.txtBatch_Num.Enabled = False
            Me.cboBatch_Num.Enabled = False

            Me.txtMember_SN.Text = Val(RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_SNO") & vbNullString, String)))

            Me.txtGender.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_CAT") & vbNullString, String))
            Call gnProc_DDL_Get(Me.cboGender, RTrim(Me.txtGender.Text))

            If IsDate(objOLEDR("TBIL_ADD_COV_MEMB_BDATE")) Then
                Me.txtMember_DOB.Text = Format(CType(objOLEDR("TBIL_ADD_COV_MEMB_BDATE"), DateTime), "dd/MM/yyyy")
            End If
            Me.txtDOB_ANB.Text = Val(objOLEDR("TBIL_ADD_COV_MEMB_AGE") & vbNullString)

            If IsDate(objOLEDR("TBIL_ADD_COV_MEMB_FROM_DT")) Then
                Me.txtStart_Date.Text = Format(CType(objOLEDR("TBIL_ADD_COV_MEMB_FROM_DT"), DateTime), "dd/MM/yyyy")
            End If
            If IsDate(objOLEDR("TBIL_ADD_COV_MEMB_TO_DT")) Then
                Me.txtEnd_Date.Text = Format(CType(objOLEDR("TBIL_ADD_COV_MEMB_TO_DT"), DateTime), "dd/MM/yyyy")
            End If

            Me.txtPrem_Period_Yr.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_TENOR") & vbNullString, String))
            Me.txtDesignation_Name.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_DESIG") & vbNullString, String))
            Me.txtMember_Name.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_NAME") & vbNullString, String))

            If Val(RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_SA_FACTOR") & vbNullString, String))) <> 0 Then
                Me.txtPrem_SA_Factor.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_SA_FACTOR") & vbNullString, String))
            End If

            Me.txtTotal_Emolument.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_TOT_EMOLUMENT") & vbNullString, String))
            Me.txtSum_Assured.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_SA") & vbNullString, String))

            Me.txtMedical_YN.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_MEDICAL_YN") & vbNullString, String))
            Call gnProc_DDL_Get(Me.cboMedical_YN, RTrim(Me.txtMedical_YN.Text))

            Call gnProc_DDL_Get(Me.cboPrem_Rate_Code, RTrim(Me.txtPrem_Rate_Code.Text))

            Me.txtPrem_Rate.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_RATE") & vbNullString, String))
            Me.txtPrem_Rate_Per.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_RATE_PER") & vbNullString, String))
            Me.txtPrem_Amt.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_PREM") & vbNullString, String))
            Me.txtPrem_Amt_Prorata.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_PRO_RATE_PREM") & vbNullString, String))
            Me.txtLoad_amt.Text = RTrim(CType(objOLEDR("TBIL_ADD_COV_MEMB_LOAD") & vbNullString, String))
    ...
    Protected Sub cmdPrev_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdPrev.Click
        ...
        pvURL = "prg_li_grp_poly_add_cover.aspx?q=x"
