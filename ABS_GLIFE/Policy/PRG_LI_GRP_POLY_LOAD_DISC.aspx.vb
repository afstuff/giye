...
Partial Class Policy_PRG_LI_GRP_POLY_LOAD_DISC
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        strTableName = "TBIL_GRP_POLICY_DISCT_LOAD"
    ...
    Protected Sub DoProc_Load_Disc_Type_Change()
        ...
        If Trim(Me.txtLoad_Disc_Type.Text) = "L" Then
            Call gnProc_Populate_Box("GL_CODE_LIST_UND", "004", Me.cboLoad_Disc_Num)
        ElseIf Trim(Me.txtLoad_Disc_Type.Text) = "D" Then
            Call gnProc_Populate_Box("GL_CODE_LIST_UND", "005", Me.cboLoad_Disc_Num)
        Else
            Me.cboLoad_Disc_Num.Items.Clear()
        End If
    End Sub
    ...
    Private Sub Proc_DataBind()
        ...
        strSQL = "SPGL_GET_POLICY_DISCT_LOAD"
    ...
    Private Sub Proc_DoSave()
        ...
        If Trim(Me.txtLoad_Disc_Prem_Rate_TypeNum.Text) = "A" Then
            Me.txtCover_Rate_Per.Text = "0"
            Me.txtCover_Rate.Text = "0"
            Me.txtLoad_Disc_Percent.Text = "0"
        End If
        If Trim(Me.txtLoad_Disc_Prem_Rate_TypeNum.Text) = "F" Or Trim(Me.txtLoad_Disc_Prem_Rate_TypeNum.Text) = "R" Then
            Me.txtLoad_Disc_Amt.Text = "0"
            Me.lblCover_Rate_Per.Enabled = True
            Me.cboCover_Rate_Per.Enabled = True
        End If
        ...
        Call MOD_GEN.gnGET_SelectedItem(Me.cboLoad_Disc_Mode, Me.txtLoad_Disc_Mode, Me.txtLoad_Disc_ModeName, Me.lblMsg)
        If Me.txtLoad_Disc_Mode.Text = "" Then
            Me.lblMsg.Text = "Missing " & Me.lblLoad_Disc_Mode.Text
            FirstMsg = "Javascript:alert('" & Me.lblMsg.Text & "')"
            Exit Sub
        End If
        ...
        Dim myLoad_AmtX As Double = 0
        Dim myLoad_Amt_Add_Mem As Double = 0

        Select Case UCase(Trim(Me.txtLoad_Disc_Type.Text))
            Case "L"    'loading
                Select Case UCase(Trim(Me.txtLoad_Disc_Mode.Text))
                    Case "I"    'for individual
                        myLoad_AmtX = Proc_DoLoading(objOLEConn)
                        myLoad_Amt_Add_Mem = Proc_DoLoading_Add_Member(objOLEConn)
        ...
        Try
            ...
            If obj_DT.Rows.Count = 0 Then
                ...
                '
                drNewRow("TBIL_POL_DISC_MDLE") = RTrim("G")
                ...
                drNewRow("TBIL_POL_DISC_LOAD_INV_GRP") = RTrim(Me.txtLoad_Disc_Mode.Text)
            ...
            Else
                ...
                With obj_DT
                    ...
                    .Rows(0)("TBIL_POL_DISC_MDLE") = RTrim("G")
                    ...
                    .Rows(0)("TBIL_POL_DISC_LOAD_INV_GRP") = RTrim(Me.txtLoad_Disc_Mode.Text)
    ...
    Private Function Proc_DoLoading(ByVal pv_objOLEConn As OleDbConnection) As Double
        ...
        myPercent = Val(Me.txtLoad_Disc_Percent.Text)
        myRate = Val(Me.txtCover_Rate.Text)
        myPer = Val(Me.txtCover_Rate_Per.Text)
...
Proc_End_Rtn:
    ...
    Private Function Proc_DoLoading_Add_Member(ByVal pv_objOLEConn As OleDbConnection) As Double
...
Proc_End_Rtn:
    ...
    Private Function Proc_DoOpenRecord(ByVal FVstrGetType As String, ByVal FVstrRefNum As String, Optional ByVal FVstrRecNo As String = "", Optional ByVal strSearchByWhat As String = "FILE_NUM") As String
        ...
        If (objOLEDR.Read()) Then
            ...
            Me.txtLoad_Disc_Mode.Text = RTrim(CType(objOLEDR("TBIL_POL_DISC_LOAD_INV_GRP") & vbNullString, String))
            Call gnProc_DDL_Get(Me.cboLoad_Disc_Mode, RTrim(Me.txtLoad_Disc_Mode.Text))
    ...
    Protected Sub cmdPrev_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdPrev.Click
        ...
        Dim pvURL As String = ""
        pvURL = "prg_li_indv_poly_charges.aspx?q=x"
        pvURL = "prg_li_grp_poly_charges.aspx?q=x"
