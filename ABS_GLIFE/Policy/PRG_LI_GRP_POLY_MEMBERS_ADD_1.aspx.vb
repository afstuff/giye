...
Partial Class Policy_PRG_LI_GRP_POLY_MEMBERS_ADD
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim myBat As String = ""
        Try
            myBat = CType(Request.QueryString("optbatno"), String)
            'myBat = CType(Session("optbatno"), String)
        Catch ex As Exception
            myBat = ""
        End Try

        If Trim(myBat) <> "" Then
            Me.txtBatch_Num.Text = myBat
            Me.txtBatch_Num.Enabled = False
            Me.cboBatch_Num.Enabled = False
            Me.cmdGetBatch.Enabled = False
        End If
    ...
    Private Sub Proc_DoNew()
        ...
        Call Proc_DDL_Get(Me.cboBatch_Num, RTrim("*"))
        Me.cboBatch_Num.Enabled = True
    ...
    Private Sub Proc_DoSave_Upload()
        ...
        mystr_sn_param = txtBatch_Num.Text
...
MyLoop_Start:
        ...
        If Trim(sFT) = "Y" Then
            ...
            mystr_sql = mystr_sql & " and TBIL_POL_MEMB_BATCH_NO = '" & RTrim(Me.txtBatch_Num.Text) & "'"
            mystr_sql = mystr_sql & " and TBIL_POL_MEMB_STATUS = '" & RTrim("Q") & "'"
            myole_cmd = New OleDbCommand(mystr_sql, myole_con)
            myole_cmd.CommandType = CommandType.Text
            ...
            ' get the last member serial no
            mystr_sql = ""
            mystr_sql = "select top 1 TBIL_POL_MEMB_SNO from TBIL_GRP_POLICY_MEMBERS"
            mystr_sql = mystr_sql & " where TBIL_POL_MEMB_FILE_NO = '" & RTrim(my_File_Num) & "'"
            mystr_sql = mystr_sql & " and TBIL_POL_MEMB_PROP_NO = '" & RTrim(my_Prop_Num) & "'"
            'mystr_sql = mystr_sql & " and TBIL_POL_MEMB_POLY_NO = '" & RTrim(my_Poly_Num) & "'"
            'mystr_sql = mystr_sql & " and TBIL_POL_MEMB_BATCH_NO = '" & RTrim(Me.txtBatch_Num.Text) & "'"
            mystr_sql = mystr_sql & " AND ISNUMERIC(ISNULL(TBIL_POL_MEMB_SNO,'0')) = 1"
            mystr_sql = mystr_sql & " ORDER BY TBIL_POL_MEMB_SNO DESC"
            ...
            If (myole_dr.Read()) Then
                my_SNo_Int = Val(RTrim(CType(myole_dr("TBIL_POL_MEMB_SNO") & vbNullString, String)))
            Else
                my_SNo_Int = Val(0)
            End If
        ...
        my_SNo_Int = my_SNo_Int + 1
        my_SNo = my_SNo_Int.ToString
        ...
        If Val(Me.txtPrem_SA_Factor.Text) <> 0 Then
            dblTotal_SA = CDbl(Trim(my_Total_Salary)) * Val(Trim(Me.txtPrem_SA_Factor.Text))
        End If
        ...
        mystr_sql = "SPGL_TBIL_GRP_POLICY_MEMBERS_ADD"
        ...
        myole_cmd.Parameters.AddWithValue("@p14A", CDbl(Trim(Me.txtPrem_SA_Factor.Text)))
...
Proc_Skip_ANB:
        ...
        strSQL = strSQL & " WHERE TBIL_POL_MEMB_PROP_NO = '" & RTrim(txtQuote_Num.Text) & "'"
        strSQL = strSQL & " AND TBIL_POL_MEMB_POLY_NO = '" & RTrim(txtPolNum.Text) & "'"
